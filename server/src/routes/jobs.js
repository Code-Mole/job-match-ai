...
router.get("/", async (req, res, next) => {
  try {
    ...
    if (salaryMin || salaryMax) {
      const salaryParts = [];
      if (salaryMin)
        salaryParts.push({ salaryMax: { $gte: Number(salaryMin) } });
      if (salaryMax)
        salaryParts.push({ salaryMin: { $lte: Number(salaryMax) } });
...
router.get("/match", protect, async (req, res, next) => {
  try {
    const user = await User.findById(req.user._id)
      .select(
        "skills cvText yearsExp cvRoles skillStrengths cvUploadedAt cvParsed",
      )
      .lean();
    if (!user) {
      return res
    ...
    const topN = Math.min(
      50,
      Math.max(1, Number(req.query.top_n) || Number(limitNum) || 25),
    );
    ...
    if (err.code === "ECONNREFUSED") {
      return res.status(503).json({
        success: false,
        message:
          "AI matching service is unavailable. Please try again shortly.",
      });
    }
    next(err);
  }
...
router.post("/seed", async (req, res, next) => {
  try {
    ...
    const sampleJobs = [
      {
        ...
        industry: "Fintech",
        description:
          "We are looking for a Senior Frontend Developer to join our Payments team at Stripe. You will build and maintain our web applications used by millions of businesses globally. You will work closely with product designers and backend engineers to deliver exceptional user experiences.",
        requirements: [
      ...
      },
      {
        ...
        description:
          "Join Vercel as a Full Stack Engineer and help us build the future of web development. You will work on our deployment platform, edge network, and developer tooling that powers millions of websites.",
        requirements: [
      ...
      },
      {
        ...
        description:
          "Linear is building the new standard for project management. We are looking for a React Developer to help us build a product that thousands of teams depend on daily.",
        requirements: [
          "React and hooks expertise",
          "GraphQL experience",
      ...
      },
      {
        ...
        description:
          "Figma is looking for a UI Engineer to help us build one of the most complex web applications in existence. You will work on the canvas, properties panel, and plugin ecosystem.",
        requirements: [
      ...
      },
      {
        ...
        industry: "Database",
        description:
          "Join PlanetScale to work on the world's most scalable MySQL platform. You will build the infrastructure and APIs that power our database-as-a-service product.",
        requirements: [
          "6+ years backend experience",
          "Go or Rust experience",
          "Distributed systems knowledge",
          "MySQL internals understanding",
        ],
        responsibilities: [
      ...
      },
      {
        ...
        industry: "AI/ML",
        description:
          "Hugging Face is the AI community building the future. Join us to work on open-source ML tools, models, and infrastructure used by millions of researchers and developers.",
        requirements: [
          "Strong Python skills",
          "PyTorch or TensorFlow",
          "NLP or CV experience",
      ...
      },
      {
        ...
        description:
          "Cloudflare is building a better internet. We need DevOps Engineers to maintain and improve the infrastructure that serves millions of requests per second.",
        requirements: [
      ...
      },
      {
        ...
        industry: "Productivity Software",
        description:
          "Notion is on a mission to make toolmaking ubiquitous. We are looking for a Product Designer to help design the next generation of our all-in-one workspace.",
        requirements: [
    ...
    res.json({
      success: true,
      message: `Seeded ${inserted.length} jobs.`,
...
router.post("/:id/apply", protect, async (req, res, next) => {
  try {
    ...
    const already = user.appliedJobs.find(
      (a) => a.job.toString() === req.params.id,
    );
    if (already)
      return res.json({
        success: true,
